...
function TweetCard({
  ...
  return (
    <li className="flex gap-4 border-b px-4 py-4">
      ...
      <div className="flex-gro flex flex-col">
...
function HeartButton({ likedByMe, likeCount }: HeartButtonProps) {
  ...
  if (session.status !== "authenticated") {
    return (
      <div className="mb- mt-1 flex items-center gap-3 self-start text-gray-500">
        <HeartIcon />
        <span>{likeCount}</span>
      </div>
    );
    ...
    >
      <HeartIcon
        className={`transition-colors duration-200 ${
          likedByMe
            ? "fill-red-500"
            : "group-focus-visible-fill-red-500 fill-gray-500 group-hover:fill-red-500"
        }`}
      />
      <span>{likeCount}</span>
    </button>
